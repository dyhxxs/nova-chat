...
- **多会话并发**：每个对话拥有独立 WebSocket、取消控制器和请求状态；切换到另一个对话不再锁住输入，也不会把增量内容写错会话。
- **更稳定的流式回复**：握手前可安全回退 REST；心跳、停滞检测、断线错误和 partial 内容保留完整；已开始输出后不会自动重放而导致重复回答或重复计费。
- **正确上下文**：失败、取消、流式未完成的助手消息不会进入下一轮；图片/PDF 引用会随历史保留；历史按字符预算裁剪。
...
- **自动生图与模型降级**：用户说“生图/生成图片/画图”等表达时自动调用 `/images/generations`；图片模型和文本模型分别在各自候选中降级，已产生部分文字后不会重复切换。
